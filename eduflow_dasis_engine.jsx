...
import { 
  ...
  Cpu, Award, Landmark, FileUp, Power, RefreshCw
} from 'lucide-react';

/**
 * EduFlow Dasis - Real Engine
 * Created by: INISIAL TH
 * Versi: 2025.2.0 (Full Integration)
 */
...
const App = () => {
  ...
  const handleGoogleAuth = () => {
    ...
    const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${config.clientId}&redirect_uri=${window.location.origin}&response_type=token&scope=${encodeURIComponent(scope)}`;
    window.location.assign(authUrl);
  };

  // Tangkap Token dari URL setelah Redirect
  useEffect(() => {
  ...
  const uploadToDrive = async (file, fileName) => {
    ...
    if (!response.ok) throw new Error('Gagal upload ke Drive');
    return await response.json();
  };

  // --- API LOGIC: OCR VISION ---
  ...
  const saveToSheets = async (row) => {
    ...
    if (!response.ok) throw new Error('Gagal simpan ke Sheets');
    return await response.json();
  };
  ...
  const runAutomation = async (e) => {
    ...
    if (!studentData.nama || !studentData.kk) {
      showNotify('error', 'Nama & KK wajib diisi.');
      return;
    }
    ...
    try {
      ...
      const row = [timestamp, studentData.nama, 'KK', ocrResult.substring(0, 500), driveRes.webViewLink];
      await saveToSheets(row);

      const newLog = { id: Date.now(), nama: studentData.nama, status: 'Berhasil', waktu: timestamp };
      setLogs(prev => [newLog, ...prev]);
      showNotify('success', 'Data Berhasil Diproses & Disimpan!');
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col antialiased text-left">
      ...
      <nav className="bg-white/90 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-extrabold text-xl tracking-tight text-slate-900 leading-none uppercase">EduFlow Dasis</h1>
            <p className="text-[9px] text-slate-400 uppercase font-bold italic mt-1">Standalone Engine by INISIAL TH</p>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto p-6 md:p-10 flex-grow w-full">
        {view === 'login' && !isAuthenticated && (
          <div className="max-w-sm mx-auto mt-20 bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 text-center animate-in fade-in duration-300">
            ...
            <p className="text-slate-400 text-xs mb-8 font-semibold uppercase">Area Terbatas Admin</p>
            ...
            <div className="lg:col-span-8 bg-white p-8 md:p-14 rounded-[3rem] border border-slate-100 shadow-2xl">
              <div className="flex items-center gap-5 mb-12 text-left border-b border-slate-50 pb-8">
                ...
                <div>
                  ...
                  <p className="text-slate-400 font-bold uppercase text-[11px] mt-2">Mesin Otomasi Digital v2025</p>
                </div>
              </div>
              
              <form onSubmit={runAutomation} className="space-y-12">
                <div className="space-y-4">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-2 block">Identitas Siswa</label>
                  <input type="text" className="w-full px-8 py-6 rounded-3xl border border-slate-100 outline-none text-2xl font-bold focus:ring-8 focus:ring-blue-50 transition-all bg-slate-50" placeholder="Nama Sesuai Dokumen..." value={studentData.nama} onChange={(e) => setStudentData({...studentData, nama: e.target.value})} />
                </div>
        ...
        {view === 'admin' && isAuthenticated && (
          <div className="bg-white rounded-[3rem] border border-slate-100 overflow-hidden shadow-2xl">
            <div className="p-12 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <a href={`https://docs.google.com/spreadsheets/d/${config.spreadsheetId}`} target="_blank" className="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black text-xs hover:bg-emerald-700 flex items-center gap-3 transition-all">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {logs.length > 0 ? logs.map(log => (
                    ...
                    <tr><td colSpan="3" className="px-12 py-20 text-center text-slate-300 font-bold italic">Belum ada data di sesi ini.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        {view === 'settings' && isAuthenticated && (
          <div className="max-w-3xl mx-auto bg-white p-12 rounded-[3rem] shadow-2xl border border-slate-100 text-left">
            <h2 className="text-3xl font-black text-slate-900 mb-10 flex items-center gap-4"><Settings className="w-8 h-8" /> Konfigurasi Mesin</h2>
            
            <div className="mb-8 p-6 bg-blue-50 border border-blue-100 rounded-3xl flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="text-left">
                <p className="font-bold text-blue-900">Hubungkan Google Cloud</p>
                <p className="text-xs text-blue-700 italic">Diperlukan agar mesin bisa menulis ke Drive & Sheets.</p>
              </div>
              <button onClick={handleGoogleAuth} className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-blue-700 shadow-lg transition-all">
            ...
            <div className="space-y-8">
              ...
              <ConfigInput label="Folder ID Drive" value={config.folderId} onChange={(val) => setConfig({...config, folderId: val})} />
              ...
              <div className="pt-8 border-t border-slate-100 space-y-4 text-left">
                <label className="text-[11px] font-black text-blue-600 uppercase tracking-widest block">🔒 Password Panel Admin</label>
                <input type="text" className="w-full px-6 py-5 rounded-2xl border border-blue-100 outline-none text-lg font-bold bg-blue-50 focus:ring-8 focus:ring-blue-100 transition-all" value={config.masterPassword} onChange={(e) => setConfig({...config, masterPassword: e.target.value})} />
              </div>
              
              <button onClick={() => {localStorage.setItem('eduflow_config_v2025', JSON.stringify(config)); showNotify('success', 'Disimpan!');}} className="w-full bg-slate-900 text-white py-6 rounded-3xl font-black uppercase tracking-[0.3em] shadow-2xl hover:bg-black transition-all flex items-center justify-center gap-3"><Save className="w-5 h-5" /> Simpan Permanen</button>
            </div>
          </div>
        )}
      </main>
      ...
      <footer className="py-12 border-t border-slate-100 bg-white/50 text-center">
        <div className="max-w-6xl mx-auto px-6 flex justify-between items-center">
          ...
          <button onClick={() => !isAuthenticated && setView('login')} className="opacity-10 hover:opacity-100 transition-opacity text-black text-[10px] font-bold uppercase tracking-widest">STAF SAJA</button>
        </div>
      </footer>
    </div>
  );
...
const StatusStep = ({ active, current, label, desc }) => (
  <div className={`flex items-start gap-6 transition-all duration-700 ${active ? 'opacity-100' : 'opacity-20'}`}>
    <div className={`w-12 h-12 rounded-[1.25rem] flex items-center justify-center flex-shrink-0 transition-all ${current ? 'bg-yellow-400 animate-pulse ring-8 ring-yellow-400/20 shadow-lg' : active ? 'bg-emerald-500' : 'bg-slate-800'}`}>
...
const ConfigInput = ({ label, value, onChange }) => (
  <div className="text-left">
    ...
    <input type="text" className="w-full px-6 py-5 rounded-2xl border border-slate-100 outline-none text-sm font-mono bg-white focus:ring-4 focus:ring-slate-50 transition-all" value={value} onChange={(e) => onChange(e.target.value)} />
  </div>
);
